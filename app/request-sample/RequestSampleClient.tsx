...
/** Slot aset diselesaikan di server (page.tsx) lalu dioper ke sini: `node:fs`
 *  tidak tersedia di Client Component. */
type Props = { art: string | null };

export default function RequestSampleClient({ art }: Props) {
    ...
    <main>
      ...
      <section className="form-page shell">
        <aside className="form-intro">
          ...
          {/* Kardus terbuka dengan kartu konten keluar darinya. Menggambarkan
              alur yang dijelaskan tiga langkah di bawahnya, bukan hiasan.
              Diganti otomatis kalau tim menaruh public/art/sample-box.png. */}
          <ArtSlot src={art} scene="sample" className="form-intro-art" />

          {/* Ubin ikon, tapi TANPA chevron seperti di mockup: baris ini bukan
              tautan, dan chevron akan menjanjikan aksi yang tidak ada. */}
          <div className="steps">
            <div>
        ...
        {/*
          Ketiga klaim di bawah SUDAH dinyatakan di tempat lain pada situs ini,
          "gratis" di /daftar, "alamat dipakai ulang" di kartu masuk halaman ini,
          dan "diperiksa tim" di langkah 02. Tidak ada benefit baru yang dikarang;
          GENERAL-WRITING-COPY-STYLE.md §9 melarangnya.
        */}
        <ul className="benefit-strip">
          <li>
            <i className="icon-tile icon-tile-sm"><Icon name="gift" /></i>
            <span><b>Gratis untuk creator Haluan</b>Tidak ada biaya admin maupun potongan.</span>
          </li>
          <li>
            <i className="icon-tile icon-tile-sm"><Icon name="clipboard-text" /></i>
            <span><b>Alamat dipakai ulang</b>Tidak perlu mengisi ulang formulir tiap request.</span>
          </li>
          <li>
            <i className="icon-tile icon-tile-sm"><Icon name="shield-check" /></i>
            <span><b>Diperiksa sebelum dikirim</b>Profil, brief, dan kuota dicek tim Haluan.</span>
          </li>
        </ul>
      </section>
    </main>
